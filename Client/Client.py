...
import threading
import time
from Client import Decoder
from Client import Encoder
from tkinter import *
...
class Client:
    def __init__(self):
        ...
        # GUI용 필드
        self.window = Tk()
        self.window.geometry('500x450+100+100')
        ...
        self.announcement = tk.Text(self.chat_frame, width=50, height=5)
        self.announcement.config(state=tk.DISABLED)
    ...
    def receiver(self):
        while not self.login:
            ...
            self.decoder.decode(nickname_check, self)
            if not self.login:
                print('중복된 닉네임이 존재합니다.')

        while True:
            message = self.socket.recv(1024).decode()
            self.decoder.decode(message, self)
    ...
    def initialize_GUI(self):
        ...
        self.announcement.grid(row=0, column=0, padx=10, pady=10)
        self.content_frame.grid(row=1, column=0)
        tk.Listbox(self.chat_frame, height=10, width=15, listvariable=self.member_list).grid(row=1, column=1, padx=5, pady=5)
        tk.Entry(self.chat_frame, width=50, textvariable=self.input_msg).grid(row=2, column=0, padx=5, pady=5)
        tk.Button(self.chat_frame, text='Send', command=self.send_message).grid(row=2, column=1, padx=5, pady=5)
        tk.Button(self.chat_frame, text='Exit', command=self.exit).grid(row=3, column=1, padx=5, pady=5)
